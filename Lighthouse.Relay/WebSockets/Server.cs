...
public partial class Server
{
  ...
  /// <param name="host">The listen endpoint of the server</param>
  public Server(string host)
  {
    var hostArr = host.Split(":");
    var endPoint = new IPEndPoint(IPAddress.Parse(hostArr[0]), int.Parse(hostArr[1]));
  ...
  public async Task SendMessage(Client Client, string Data)
  {
    ...
    await Client.GetSocket().SendAsync(frameMessage);
